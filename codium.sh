...
if [ -z "${CONTAINER_ID}" ]; then
    /usr/bin/distrobox-enter -n vscodium-distrobox -- '/usr/bin/codium' --no-url-handler --enable-features=WaylandWindowDecorations --ozone-platform=wayland --class=VSCodium "$@" 2> /dev/null | grep -v '^Warning:'
